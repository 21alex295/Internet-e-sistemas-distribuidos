...
public abstract class AbstractSqlViajeDao implements SqlViajeDao {
	...
	public void actualizar(Connection connection, Viaje viaje) throws InstanceNotFoundException{
		String queryString = "UPDATE Viaje SET idConductor = ?, origen = ?, destino = ?,"
				+" idUsuario = ?, tarjetaCredito = ?, fechaReserva = ?, puntuacion = ?";
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(queryString)){
            ...
            Timestamp fechaReserva = viaje.getFechaReserva() != null ? new Timestamp(
                    viaje.getFechaReserva().getTime().getTime()) : null;
